...
void main() {
  ...
  group('test field changed', () {
    test('Adding normal text only modifies text', () async {
      final bloc = createBloc();
      bloc
        ..add(const AddDiaryEntryEvent.initialize(mentionableList: [Mentionable("name")]))
        ..add(
          txtChngEvent("t", 1),
        );
      await expectLater(
          bloc.stream.skip(1),
          emitsInOrder([
            predicate<AddDiaryEntryState>((actual) {
              final expected = AddDiaryEntryState.initial().copyWith(
                  entryField: DiaryEntryApplication(
                    entry: DiaryEntry.empty().copyWith(text: "t", mentionList: []),
                  ),
                  mentionListManager: mentionListManager);

              return verifyBlocs(expected, actual);
            })
          ]));
    });
    ...
    //     bloc.add(
    //         const AddDiaryEntryEvent.initialize(mentionableList: [Mentionable("name")]));
    //     addtxtToBloc(bloc, "@a");
    //   },
    //   skip: 2,
    //   expect: () => [
    ...
    //               .copyWith(text: "@name", mentionList: [firstNameMention]),
    //         ),
    ...
    //         entryField: DiaryEntryApplication(
    //           entry: DiaryEntry.empty()
    //               .copyWith(text: "@name2", mentionList: [firstNameMention]),
    //         ),
    //         mentionListManager: const MentionListManager(
    //             mentionList: [Mentionable("name"), Mentionable("name2")]))
    //   ],
  ...
  group("On select mention event", () {
    test('If there is no active mention to complete do nothing', () async {
      final bloc = createBloc();
      bloc.add(
          const AddDiaryEntryEvent.onSelectMention(iMentionable: Mentionable("name")));
    ...
    //
    //   bloc.add(const AddDiaryEntryEvent.onSelectMention(iMentionable: mentionable));
    //
    //   await expectLater(
    //     bloc.stream,
    //     emitsInOrder([
    ...
    // });
    //
    // test('If there is an active mention complete text with prefix and suffix', () async {
    //   const mentionable = Mentionable("name");
    //
    //   final bloc = createBloc();
    //   bloc.emit(AddDiaryEntryState.initial().copyWith(
  ...
  group('Remove mention event test', () {
        ...
        act: (AddDiaryEntryBloc bloc) {
          ...
          bloc.add(
            const AddDiaryEntryEvent.removeMention(
              mention: Mention(startPos: 0, endPos: 5, iMentionable: Mentionable("name")),
              baseOffset: 0,
              extentOffset: 0,
            ),
          );
    ...
    //       entryField: DiaryEntryApplication(
    //           entry:
    //               DiaryEntry.empty().copyWith(text: "<=@name=>", mentionList: [mention])),
    //       mentionListManager: const MentionListManager(mentionList: [mentionable])));
    //
...
bool verifyBlocs(AddDiaryEntryState expected, AddDiaryEntryState actual) {
  return listEquals(expected.mentionListManager.mentionList,
          actual.mentionListManager.mentionList) &&
      expected.entryField.baseOffset == actual.entryField.baseOffset &&
      expected.entryField.extentOffset == actual.entryField.extentOffset &&
      expected.entryField.selectingMention == actual.entryField.selectingMention &&
      listEquals(
          expected.entryField.entry.mentionList, actual.entryField.entry.mentionList) &&
      expected.entryField.entry.text == actual.entryField.entry.text;
}
